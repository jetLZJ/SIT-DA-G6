...
def page_cleaning_and_eda(engine: Optional[sqlalchemy.engine.Engine]):
    ...
    with st.expander('Data loading & table selection', expanded=True):
    ...
    with st.expander('Data checks and descriptive summary', expanded=True):
    ...
    with st.expander('Derived table: employed_count (notebook example)', expanded=False):
    ...
    with st.expander('Outlier inspection', expanded=False):
    ...
    with st.expander('Module 3 — Visualisations (recreated from notebook)', expanded=False):
    ...
    st.markdown('---')
    with st.expander('How this answers the Problem Statement', expanded=False):
        st.markdown(
            """
    ...
    if nb_path.exists():
        with st.expander('Download assets', expanded=False):
